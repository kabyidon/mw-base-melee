...
hook.Add( "Initialize", "ReplaceMeleeBehaviourModuleForVJL4D2", function()
    ...
    -- Inspired by Clementine's MW Base Plus  
    ...
    function MW:MeleeBehaviourModule()
        ...
        local function Flinch(v)
            ...
            for _,v in pairs(tbl) do
                if v:Distance(self:GetPos()) < nearestDist then
                    nearestDist = v:Distance(self:GetPos())
                    cPos = v
                end
            end
            ...
            if v.OnShoved then
                anim = v:OnShoved(anim,self.Owner)
            end
        ...
        local function GetNearestPoint(argent,SameZ)
                ...
                if (CurTime() > self:GetNextMeleeTime()) then
                    ...
                    for _,v in pairs(ents.FindInSphere(self:GetOwner():GetShootPos(), meleeRange)) do
                        if v:IsNPC() && self:GetOwner():Visible(v) && v != self && v != owner then
                            ...
                            dmgInfo:SetDamagePosition(GetNearestPoint(v).MyPosition)
